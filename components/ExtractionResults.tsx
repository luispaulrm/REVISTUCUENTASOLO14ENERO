  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-end border-b-2 border-slate-100 pb-4">
        <div>
          <h2 className="text-2xl font-black text-slate-800 tracking-tighter uppercase">{data.clinicName || 'Entidad Clínica'}</h2>
          <div className="flex items-center gap-2 mt-1">
            <span className="text-[10px] font-bold px-2 py-0.5 bg-slate-100 text-slate-500 rounded uppercase">Paciente</span>
            <p className="text-sm text-slate-600 font-medium">{data.patientName || 'N/A'}</p>
            <span className="text-slate-300 mx-1">•</span>
            <span className="text-[10px] font-bold px-2 py-0.5 bg-slate-100 text-slate-500 rounded uppercase">Doc</span>
        ...
        <div className="text-right">
          ...
          <p className="text-slate-900 font-bold bg-slate-50 px-3 py-1 rounded-lg border border-slate-100 inline-block">{data.date || 'N/A'}</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {data.sections.map((section, sIdx) => {
          ...
          return (
            <div key={sIdx} className={`bg-white border rounded-2xl overflow-hidden transition-all shadow-sm ${section.hasSectionError ? 'border-rose-300 ring-2 ring-rose-50' : 'border-slate-200'}`}>
              <button
                onClick={() => toggleSection(sIdx)}
                className={`w-full flex items-center justify-between p-4 hover:bg-slate-50 transition-colors ${section.hasSectionError ? 'bg-rose-50/40' : ''}`}
              >
                  ...
                  {section.hasSectionError && (
                    <div className="flex items-center gap-2">
                      {section.isTaxConfusion ? (
                        <span className="flex items-center gap-1 text-[9px] px-2 py-0.5 bg-amber-500 text-white rounded-full font-black uppercase">
                          <Info size={10} /> Impuestos/Recargos (Chile)
                        </span>
                      ) : section.isUnjustifiedCharge ? (
                        <span className="flex items-center gap-1 text-[9px] px-2 py-0.5 bg-rose-600 text-white rounded-full font-black animate-pulse uppercase ring-2 ring-rose-200">
                          <AlertTriangle size={10} /> Cobro No Justificado
                        </span>
                      ) : (
                        <span className="flex items-center gap-1 text-[9px] px-2 py-0.5 bg-slate-600 text-white rounded-full font-black uppercase">
              ...
              {expanded[sIdx] && (
                <div className="px-4 pb-4">
                  {section.isUnjustifiedCharge && (
                    <div className="mb-4 p-4 bg-rose-50 border border-rose-200 rounded-2xl flex items-start gap-4 animate-in fade-in slide-in-from-top-2">
                      ...
                      <div className="space-y-2">
                        <p className="text-sm font-black text-rose-900 uppercase tracking-tighter flex items-center gap-2">
                          Auditoría Forense: Cobro No Justificado
                        </p>
                        <div className="text-[11px] text-rose-800 leading-relaxed font-medium space-y-1">
                          ...
                          <p className="pt-1 mt-1 border-t border-rose-200 text-rose-900 font-bold italic">
                  ...
                  {section.isTaxConfusion && (
                    <div className="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-start gap-4">
                      <div className="p-3 bg-amber-100 text-amber-600 rounded-xl shrink-0 shadow-sm border border-amber-200">
                        <Info size={20} />
                      </div>
                      <div className="space-y-2">
                        <p className="text-sm font-black text-amber-900 uppercase tracking-tighter">Variación por Impuestos o Recargos Legales</p>
                  ...
                  <div className="overflow-x-auto rounded-xl border border-slate-100">
                    <table className="w-full text-xs text-left">
                      <thead className="bg-slate-50/50 text-slate-400 uppercase tracking-tighter font-black">
                        <tr>
                      ...
                      <tbody className="divide-y divide-slate-50">
                        {section.items.map((item, iIdx) => (
                          <tr key={iIdx} className={`group hover:bg-indigo-50/30 transition-colors ${item.hasCalculationError ? 'bg-amber-50/30' : ''}`}>
                            <td className="px-4 py-3 text-slate-400 font-mono text-[10px] font-bold">
                              {item.index || iIdx + 1}
                            </td>
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                {item.hasCalculationError && (
                                  <div className="group relative">
                                    ...
                                    <div className="hidden group-hover:block absolute left-0 top-5 z-50 w-72 p-4 bg-slate-900 text-white text-[10px] rounded-2xl shadow-2xl border border-slate-700 leading-relaxed animate-in fade-in zoom-in-95">
                                      ...
                                      <div className="space-y-1 font-mono text-slate-300">
                                        ...
                                        <p className="text-emerald-400 font-bold">Esperado: {formatCurrency(item.calculatedTotal)}</p>
                                        <p className="text-rose-400 font-bold">En Cuenta: {formatCurrency(item.total)}</p>
                                      </div>
                                      <p className="mt-3 text-slate-400 italic border-t border-slate-800 pt-2">
                      ...
                      <tfoot className="bg-slate-50/30 border-t-2 border-slate-100">
                        <tr>
                          ...
                          <td className={`px-4 py-4 text-right font-black text-base ${section.hasSectionError ? 'text-rose-600' : 'text-indigo-600'}`}>
                            {formatCurrency(section.calculatedSectionTotal)}
                          </td>
                        </tr>
                        {section.hasSectionError && (
                          <tr className={section.isUnjustifiedCharge ? 'bg-rose-50' : 'bg-amber-50/50'}>
