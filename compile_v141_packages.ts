...
function parseExceptionText(text) {
    ...
    if (match) {
        ...
        return { kind: 'PRESTADOR_EXCEPTION', raw_text: text, prestadores };
    }
    return null;
}

function toAtomicId(id) {
    const map = {
    ...
    // Strip any C2+C3 style references
    if (/C\d+\+C\d+/.test(id)) return 'COL_PREF_PCT'; // Force atomic fallback
    return map[id] || id;
}
...
function compilePackage(baseName, docTitle, page) {
    ...
    // 1. SPATIAL MAP HARDENING (v1.4.1)
    ...
    spatialMap.rows?.forEach(r => r.confidence = r.confidence || { geometry: 0.96, text: 0.95 });
    ...
    spatialMap.zones.forEach(z => {
        // FIX 2: Kill-switch → ZONE_EXCLUSION
    ...
    // 2. ASSIGNMENTS HARDENING (v1.4.1)
    ...
    if (baseName === 'ple847') {
        ...
        raw.forEach(rowGroup => {
            Object.keys(rowGroup.results).forEach(colId => {
                ...
                const atoms = [{ type: 'RULE', key: res.type, value: res.value, unit: res.unit }];

                // FIX 4: ConditionAtom for exception text
                if (res.condition || /excepto/i.test(res.value?.toString() || '')) {
                    const parsed = parseExceptionText(res.condition || res.value?.toString() || '');
                    if (parsed) atoms.push({ type: 'CONDITION', ...parsed });
                }

                flat.push({
                    assignment_id: `a_${rowGroup.row_id}_${atomicCol}`,
                    row_id: rowGroup.row_id,
                    column_id: atomicCol,
                    pointer: {
                        type: res.type === 'ZONE_ASSIGNED' ? 'ZONE_REFERENCE' : 'TEXT_DIRECT_CELL',
    ...
    const finalized = raw.map(a => {
        ...
        const status = isKillSwitch ? 'EXCLUDED' :
            (a.pointer?.type === 'TEXT_DIRECT' || a.pointer?.type === 'TEXT_DIRECT_CELL' ? 'ACTIVE_TEXT_DIRECT' : 'ACTIVE');

        // FIX 3: Pointer type distinction
        const pointerType = a.pointer?.bbox ? 'TEXT_DIRECT_CELL' : (a.pointer?.target_id ? 'ZONE_REFERENCE' : 'TEXT_ECHO_HEADER');

        return {
            assignment_id: `a_${a.row_id}_${atomicCol}`.replace(/C\d+\+C\d+/g, 'ATOMIC'),
            row_id: a.row_id,
            column_id: atomicCol,
    ...
    fs.writeFileSync(`./audit_package_${baseName}_v1.4.1.json`, JSON.stringify(finalPackage, null, 2));
    console.log(`Saved audit_package_${baseName}_v1.4.1.json`);
