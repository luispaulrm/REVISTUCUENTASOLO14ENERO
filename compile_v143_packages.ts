...
// v1.4.3: Row Zone Policy Configuration
...
// v1.4.3: Header Band Policy for TEXT_ECHO promotion
...
function compilePackage(baseName: string, docTitle: string, page: number) {
    ...
    // 1. SPATIAL MAP HARDENING (v1.4.3)
    ...
    fs.writeFileSync(`./audit_package_${baseName}_v1.4.3.json`, JSON.stringify(finalPackage, null, 2));
    console.log(`Saved audit_package_${baseName}_v1.4.3.json`);
    console.log(`  Overall Status: ${finalPackage.quality_metrics.overall_status}`);
    console.log(`  QC Gates:`, finalPackage.qc_gates);
    console.log(`  Zone Coverage: ${JSON.stringify(finalPackage.zone_coverage_metrics)}`);
