...
function analyzeFamilyC() {
    const files = fs.readdirSync('.').filter(f => f.startsWith('audit_package_') && f.endsWith('_v1.5.0.json'));
    const familyA_sig = 'COL_LE_PCT|COL_LE_TOPE_ANUAL|COL_LE_TOPE_EVENTO|COL_PREF_PCT|COL_PREF_TOPE_ANUAL|COL_PREF_TOPE_EVENTO|COL_PRESTACIONES';

    const members = files.filter(f => {
        const data = JSON.parse(fs.readFileSync(f, 'utf8'));
        const sig = data.spatial_map.columns.map((c: any) => c.column_id).sort().join('|');
        const hasLE = data.spatial_map.columns.some((c: any) => c.column_id === 'COL_LE_PCT');
        const rowCount = data.spatial_map.rows.length;

        const isA = sig === familyA_sig;
        const isB = !hasLE && rowCount < 18;
        return !isA && !isB;
    }).map(f => JSON.parse(fs.readFileSync(f, 'utf8')) as AuditPackage);

    console.log(`Analyzing ${members.length} Family C (Hybrid) members for anomalies...\n`);

    const targetRows = ['R_DIA_CAMA', 'R_HONORARIOS', 'R_QUIMIO', 'R_RADIOTERAPIA'];
    ...
    console.log("### FAMILY C ANOMALY REPORT ###\n");
