...
function compilePackage(baseName: string, docTitle: string, page: number) {
    console.log(`\n--- Compiling ${baseName} (v1.4.6 - Industrial Closure) ---`);
    ...
    spatialMap.zones.forEach((z: any) => {
        ...
        if (EXCEPTION_PATTERN.test(z.contains_text || '')) z.has_conditions = true;
        delete z.type;
    });
    ...
    if (baseName === 'ple847') {
        ...
        raw.forEach((rowGroup: any) => {
            Object.keys(rowGroup.results).forEach(colId => {
                ...
                // FIX 23: All assignments start as TEXT_ECHO_HEADER, will be converted to ZONE_REFERENCE
    ...
    const finalized = raw.map((a: any) => {
        ...
        // FIX 23: Only TEXT_DIRECT_CELL or TEXT_ECHO_HEADER (which will be converted)
        ...
        return {
            assignment_id: a.assignment_id || `a_${a.row_id}_${atomicCol}`,
            row_id: a.row_id,
            column_id: atomicCol,
    ...
    // v1.4.6: QC-Jurist now converts ALL TEXT_ECHO_HEADER to ZONE_REFERENCE
    const { report: jurReport, fixedAssignments, pseudoZones } = qcJurist(
        finalized,
        spatialMap,
        getDefaultHeaderBandPolicy()
    );
    ...
    fs.writeFileSync(`./audit_package_${baseName}_v1.4.6.json`, JSON.stringify(finalPackage, null, 2));
    console.log(`Saved audit_package_${baseName}_v1.4.6.json`);
    console.log(`  Overall Status: ${finalPackage.quality_metrics.overall_status}`);
    console.log(`  Promoted Echo: ${finalPackage.quality_metrics.promoted_echo_count}`);
    console.log(`  Promoted Zones: ${finalPackage.quality_metrics.promoted_zone_count}`);
