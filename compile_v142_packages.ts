...
function inferRowGroups(rows: any[], zones: any[]) {
    const groups: any[] = [];
    const headerRows = rows.filter(r => /hospitalaria|ambulatoria/i.test(r.raw_text));
    for (const h of headerRows) {
        const yStart = h.y_range[1];
        const nextHeader = headerRows.find(r => r.y_range[0] > yStart);
...
function compilePackage(baseName: string, docTitle: string, page: number) {
    ...
    // 1. SPATIAL MAP HARDENING (v1.4.2)
    ...
    if (baseName === 'ple847') {
        ...
        raw.forEach((rowGroup: any) => {
            Object.keys(rowGroup.results).forEach(colId => {
                ...
                // FIX 4/8: ConditionAtom for exception text
    ...
    const { report: jurReport, fixedAssignments } = qcJurist(finalized, spatialMap);
    ...
    fs.writeFileSync(`./audit_package_${baseName}_v1.4.2.json`, JSON.stringify(finalPackage, null, 2));
    console.log(`Saved audit_package_${baseName}_v1.4.2.json`);
    console.log(`  Overall Status: ${finalPackage.quality_metrics.overall_status}`);
    console.log(`  QC Gates:`, finalPackage.qc_gates);
    console.log(`  Intersection Rate: ${(finalPackage.intersection_pass_rate.value * 100).toFixed(1)}%`);
}
