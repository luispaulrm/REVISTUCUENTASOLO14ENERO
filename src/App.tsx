...
const App: React.FC = () => {
  ...
  const processQueue = async (queue: typeof fileQueue) => {
    ...
    for (let i = 0; i < queue.length; i++) {
      ...
      const timeoutId = setTimeout(() => {
        addLog('[SYSTEM] ⚠️ Timeout excedido (10 min). Cancelando extracción...');
        controller.abort();
      }, 600000); // 10 minutos para documentos largos
  ...
  const clearCache = () => {
    try {
      ...
      alert("🗑️ Memoria forense de Cuenta Clínica limpiada.");
    } catch (e) { }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            {status === AppStatus.SUCCESS && (
              <>
                ...
                <button onClick={() => downloadFormat('json')} className="flex items-center gap-2 px-3 py-2 bg-white text-slate-700 border border-slate-200 rounded-lg text-xs font-bold hover:bg-slate-50 transition-all shadow-sm">
                  <FileJson size={16} /> JSON
                </button>
                <button onClick={() => downloadFormat('md')} className="flex items-center gap-2 px-3 py-2 bg-white text-slate-700 border border-slate-200 rounded-lg text-xs font-bold hover:bg-slate-50 transition-all shadow-sm">
                  <FileType size={16} /> MD
                </button>
                <button
                  onClick={downloadPdf}
