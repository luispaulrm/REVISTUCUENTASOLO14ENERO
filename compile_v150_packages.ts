...
// ============================================================================
// V1.5.0 INDUSTRIAL STRICT (VPRLU FOCUS) - HELPERS
// ============================================================================
...
function toAtomicId(id: string) {
    ...
    // VPRLU Mappings
    if (/C5_C6/.test(id)) return 'COL_LE_PCT'; // Implies LE + Tope
    return map[id] || id;
}
...
function compilePackage(baseName: string, docTitle: string, page: number) {
    ...
    const processAssignment = (finalRow: string, colId: string, res: any, spatialMap: any) => {
        ...
        // FIX 35: Smart Column Re-Alignment using BBox
        // If assignment has BBox, verify intersection with assigned column. If mismatch, find correct one.
        if (res.bbox || (res.pointer && res.pointer.bbox)) {
            ...
            if (!check.valid && check.correctColumn) {
                finalCol = check.correctColumn; // Auto-Fix: Realign to geometric column
            }
        }
        ...
        // VPRLU Logic: If auto-realignment moved it to LE, but source was PREF (P1), we might have overlap.
        // idCounts handles duplicate IDs (`_2`).
    ...
    if (Array.isArray(raw) && raw.length > 0 && raw[0].results) {
        ...
        raw.forEach((a: any) => {
            ...
            // Adapt flat object to processAssignment signature
            const res = { ...a, type: a.pointer?.target_id ? 'ZONE_ASSIGNED' : 'TEXT_DIRECT' }; // Simplified adaptation
